...
\begin{example}[$\backslash$mycmd missing argument]
\begin{verbatim}
...
Example~1 shows a missing argument error. The error message \texttt{Runaway argument?} may not clearly indicate the actual problem.

\begin{example}[Nested section commands]
\begin{verbatim}
\documentclass{ctexart}
\begin{document}
...
Example~2 shows an invalid nested section command, which produces the error: ``LaTeX Error: Not allowed in LR mode.''

\begin{example}[Unclosed brace in $\backslash$frac]
\begin{verbatim}
\documentclass{ctexart}
\begin{document}
\begin{equation}
...
Example~3 shows an unclosed brace in a \verb|\frac| command, which also produces a \texttt{Runaway argument?} error.
...
\midrule
\texttt{\textbackslash AtBeginDocument} & Executes code at the beginning of the document body \\
\texttt{\textbackslash AtEndDocument} & Executes code before the document ends \\
