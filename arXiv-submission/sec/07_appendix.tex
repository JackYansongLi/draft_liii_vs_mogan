...
In fact, we have already discussed in Section~\ref{sec:eff-in-sft} the fine-tuning efficiency of Mogan's S-expressions and \LaTeX{}'s grammar. The same \LaTeX{} grammar is also adopted by Markdown for mathematical formulas, which means that the same conclusion also holds for Markdown as one of the ``Documentation-Oriented Architectures''~\cite{filho_doa_2026}.
